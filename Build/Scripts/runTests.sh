...
case ${TEST_SUITE} in
    checkRst)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name checkRst-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} "${COMMAND[@]}"
        SUITE_EXIT_CODE=$?
        ;;
    cgl)
