...
The `Core`:pn: took some decisions regarding testing that may not be obvious at first sight. This
chapter tries to answer some of the most frequent asked ones.


Why do you `Docker`:pn: everything?
===================================

Executing tests in a containerized environment like `Docker`:pn: has significant advantages over
...
authors rely on the same system dependencies as the `Core`:pn: does, local test execution is identical
...
In short: Using `Docker`:pn: creates stable an easy to replay results.


`Docker`:pn: consumes too much hard disk
========================================

Yes. If using `Docker`:pn: often and in various scenarios, it does not tend to be exactly economical
...
many `Docker`:pn: volumes and don't remove them after use. The command `docker volume list` gives an overview,
...
The script `runTests.sh` is a wrapper around `Docker Compose`:pn:. While `Docker`:pn: and `Docker Compose`:pn: are
great software, this stack has its own type of issues. In the end, runTests.sh just creates a `.env`
file read by `Docker Compose`:pn: to work around a couple of things. For instance, all tests mount the
local `Git`:pn: checkout of `Core`:pn: or an extension into one or multiple containers. Executing tests then
...
Specifying the user that runs a container is however not possible in `Docker`:pn: directly and a `long standing issue
...
Additionally, wrapping details in runTests.sh gives the `Core`:pn: the opportunity to change `Docker Compose`:pn:
details without affecting the outer caller API: We can change things on `Docker`:pn: level, adapt the script
...
For `TYPO3 Core`:pn: testing, bamboo has its own specification in :file:`Build/bamboo/src/main/java/core/PreMergeSpec.java`
(and a similar nightly setup in :file:`Build/bamboo/src/main/java/core/NightlySpec.java`) and uses
an own `Docker Compose`:pn: file in :file:`Build/testing-docker/bamboo/docker-compose.yml`. The main reason
for that is the bamboo agents are not local processes on the given hardware directly, but are also
executed in `Docker`:pn: containers to have many of them in parallel on one host.
...
For instance, they start an ad hoc PostgreSQL, a redis and a memcached container, then start a PHP 7.3 container
...
the PHP versions that Travis comes with and again runs everything in `Docker`:pn: containers. It would
...
out of scope of this document. Torturing Travis CI to fetch and run the `Docker`:pn: containers all the
...
Yes and no. We indeed consume quite some hardware to keep the `TYPO3 Core`:pn:  testing at a decent speed
...
system needs to be stable and always available, it needs to be wired into the `TYPO3`:pn: GmbH internal
...
agent cluster is busy. If you can comply with this, please mail to the `TYPO3`:pn: GmbH for details, it is highly appreciated! In the future our system demands may
hopefully shrink, maybe we're able to establish more dynamic scaling using kubernetes for instance, but
we're not there yet.


Testing is slow on `macOS`:pn:
==============================

Yes. `macOS`:pn: has issues at least in this area. We're sorry. It is how it is. There are various
issues that have not been solved in a satisfiable way when it comes to mounting bigger local
directories as `Docker`:pn: container volumes. github is full of issues with `macOS`:pn: users complaining
about poor `Docker`:pn: performance. There may be some tricks to speed that up in our tiny `TYPO3`:pn:
testing world and we're open to persons having a look at this to reduce pain for `macOS`:pn: users.
Other than that, there is little we can do. Eventually, maybe `macOS`:pn: gets it's name spacing,
file system layers and containerization right in the future? `Windows`:pn: is better and quicker in
this area by now. Let that sink in. Please keep us posted.


Can I run tests with `Windows`:pn:?
===================================
...
and experience on this, yet. Please go ahead, push patches for `Core`:pn: runTests.sh or the `docker-compose.yml`:file:
files and improve this documentation with hints about `Windows`:pn:. We already know there are some details
to take care of, a starter can be found in a `Core`:pn: patch `commit message
