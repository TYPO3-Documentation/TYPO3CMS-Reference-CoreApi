...
Acceptance testing in `TYPO3`:pn: world is about piloting a browser to click through
a frontend generated by `TYPO3`:pn: or clicking through scenarios in the backend.
...
Similar to functional testing, acceptance tests set up an isolated `TYPO3`:pn: instance
...
some helper classes to solve various needs in a typical `TYPO3`:pn: backend, it for instance
...
is again performed by :file:`runTests.sh` in `Docker`:pn: containers.
...
This allows an editor and an admin user to easily log in to the `TYPO3`:pn: backend
...
The call :php:`$I->useExistingSession('admin')` logs in an admin user into the `TYPO3`:pn:
...
This example is taken from the `Core`:pn: extension, other extensions should use their own
...
a page called "styleguide `TCA`:pn: demo" at root level, to extend that part of the tree if
