...
The `TYPO3 Core`:pn:  development has quite an impressive history on automatic testing. This chapter
...
has been committed in early 2009. The `Core`:pn: was still using SVN as version control system at this point. More
than ten years ago. The tests have later been released with `TYPO3`:pn: version 4.3 in fall 2009. The system
back then relied on the `TYPO3`:pn: extension *phpunit*. This TER extension bundled the native `phpunit package
<https://phpunit.de/>`_ and added a `TYPO3`:pn: backend module on top. It found all extensions that delivered
...
This was after `TYPO3`:pn: 4.5 times - a version that carried us for a long time. Several `TYPO3 Core`:pn:
contributors meanwhile added some hundreds of unit tests in various `Core`:pn: extensions. There was an
issue, though: Not too many persons developing the `TYPO3 Core`:pn:  cared about unit tests and executed
...
environments for open source projects. The `TYPO3 Core`:pn:  quickly started using that, a first `.travis.yml
...
builds and were able to act upon: Either fix the build or revert the patch. The `Core Team`:pn: established an
"always green" rule for `Core`:pn: development.

The Travis CI setup at this point basically created a working instance around the checked out the `Core`:pn:
to run tests: It additionally cloned a helper repository, cloned the phpunit extension, did set up
a database and other stuff, then executed the `Core`:pn: unit tests to result with "good" or "bad".
...
With frequent test execution via travis-ci more and more developers working on the `Core`:pn: were
...
relied on has been designed to run tests in the backend context of `TYPO3`:pn: as a module. The unit
...
full `TYPO3`:pn: backend context similar to the GUI based phpunit extension.
...
We ultimately learned at this time that managing system state is an essential part of the `Core`:pn:
framework. And so we started refactoring: The `Core`:pn: bootstrap has been hacked into manageable
...
language can be a huge advantage over other server-side languages. And `TYPO3`:pn: uses this a lot: If a request
is finished in `TYPO3`:pn: context, the system is "tainted" and can't be used for a second request again.

To handle this situation we came up with a functional bootstrap that creates standalone `TYPO3`:pn: instances
per test file. The system sets up a new `TYPO3`:pn: instance for each test file in an own folder in typo3temp,
...
its own. This was at `TYPO3`:pn: 6.2 times when composer was first introduced in `TYPO3`:pn: world - testing was
one of the first usages. In this process we were able to ditch the `TYPO3`:pn: specific extension based
flavor of phpunit and switched to the native version instead. This turned out to be a wise decision
since `TYPO3 Core`:pn:  testing now no longer relied on development of a third party TER extension but could
...
Functional testing gained a lot of traction: The *`DataHandler`:pn:* and various related classes in the
`TYPO3 Core`:pn:  are the most crucial and at the same time complex part of the framework. All the language,
...
So we decided to use functional tests to specify what the `DataHandler`:pn: does in which situations. There
are hundreds of tests that play through complex scenarios, example: "Add some fixture pages and
content, call `DataHandler`:pn: to create a localized version in a workspace, call `DataHandler`:pn: to merge that
workspace content into live, verify database content is as expected, set up a basic frontend, call
frontend as see if expected content is rendered." Nowadays, if changing `DataHandler`:pn: code, functional
...
which then forks another process to execute the frontend request. As a result, only hard-boiled `Core`:pn:
...
In early 2016, `Core`:pn: developers added another type of testing: Acceptance tests. Those tests use a
browser to actually click around in the backend to verify various parts of the system. For instance,
`TYPO3 Core`:pn:  had a history of breaking the installation procedure once in a while: Most `Core`:pn: developers
...
put an end to this: There are tests to install a fresh `TYPO3`:pn:, log in to the backend,
...
complex backend scenarios, verify the backend throws no `JavaScript`:pn: errors and so on.
...
is performed. Additionally the `TYPO3`:pn: backend still relies on iframes for all main modules, which again
does not simplify things. It took the `Core`:pn: development two further years to stabilize this well enough so
acceptance tests could be executed often without throwing false positives at various places. In the end
acceptance testing is another great leap forward to ensure major parts of the `TYPO3 Core`:pn:  do work as
expected.
...
added to a queue. There were `Core`:pn: code sprints were Travis reported green or red on a just
...
Travis CI was configured to only test patches that were actually merged into the `Git`:pn: main branches. So
...
So we looked for alternatives. Luckily, the `TYPO3`:pn: GmbH was founded in 2016 and got a open source license
...
issue: The `TYPO3`:pn: GmbH has a deal with the `Leibniz Rechenzentrum <https://lrz.de>`_ who grant us hardware
...
To the end of `TYPO3 Core`:pn:  v8 development the bootstrap, helper and set up code to execute `Core`:pn:
tests has been extracted from the `Core`:pn: to an own repository, the `typo3/testing-framework
<https://github.com/TYPO3/testing-framework>`_. This allowed re-using this package within
extensions to execute own tests. It however took that repository another major `Core`:pn: version
to mature well enough to easily do that. Writing and executing tests for `TYPO3`:pn: extensions is
...
Since 2016, the `TYPO3 Core`:pn:  test setup went through further changes and improvements: Various test
details were added that checked the integrity of the system. `TYPO3`:pn: v8 switched to `Doctrine`:pn: so we
...
As another important step, the `Core`:pn: developers worked on the functional test isolation again in
`TYPO3`:pn: v9: As explained above, the functional tests forked processes twice if frontend testing was
involved. With `TYPO3`:pn: v9 however, the `TYPO3 Core`:pn:  bootstrap has been heavily improved,
...
typo3temp? `TYPO3 Core`:pn:  always had the PHP constant *PATH_site* that contained the path to the document
...
of `TYPO3 Core`:pn:  v9 however is an object that is initialized during `Core`:pn: bootstrap. Next to some other
details, it also contains the path to the document root. Adding this class allowed us to ditch the
usage of PATH_site in the entire `Core`:pn:  This removed the main blocker to execute many functional test
...
we can proof that one `TYPO3`:pn: backend instance can handle many backend requests in one process - we are
sure our framework state is encapsulated well enough to allow such things. This change in the `TYPO3`:pn:
`Core`:pn: and dropping the process isolation for functional backend tests significantly simplified working
with functional tests now and debugging is much easier and improved `Core`:pn: code at the same time. This
pattern repeated often over the years: The test suites show quite well which parts of the `Core`:pn: need
attention. Working in these areas in turn improves the `Core`:pn: for everyone and allows usages that
have not been possible before.

In late 2018 another thing has been established: The *runTests.sh* script allows `Core`:pn: developers
...
that at best run in RAM, and so forth. Not too many `Core`:pn: developers went through all that to
actually run and develop tests locally. The script now hides away all that complexity and creates a
well defined and simple to use environment to run the tests, the only dependencies are recent `Docker`:pn:
and `Docker Compose`:pn: versions.


2019
====

The above milestones show that efforts in the `Core`:pn: testing area have positive effects on
`Core`:pn: and extension code and allow system usages that have not been possible before.
...
frontend scenarios. This is still ugly. It shows that calling a `TYPO3`:pn: frontend from within the
...
with the response object. Lots of details to allow this are in place since `TYPO3`:pn: v9 already, with only
...
resolved with `TYPO3`:pn: v10. As soon as this last process isolation is dropped from the functional
