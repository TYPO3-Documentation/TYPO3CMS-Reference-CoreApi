...
This chapter covers the steps required to install TYPO3 using
`Composer dependency manager <https://getcomposer.org>`_. Composer-based setups
are ideal for advanced projects or team-based workflows. They are common in
professional environments with development teams. Extensions are installed via
Packagist (not from the `TYPO3 Extension Repository (TER) <https://extensions.typo3.org>`_),
providing more flexibility in dependency management, better integration with
version control, and easier environment automation than a :ref:`Classic mode
...
A :ref:`CLI command <t3coreapi:symfony-console-commands>` `setup` can be used
as an alternative to the existing :abbr:`GUI (Graphical User Interface)`-based
web installer.
