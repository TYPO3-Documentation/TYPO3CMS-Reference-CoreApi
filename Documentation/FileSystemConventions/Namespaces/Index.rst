...
#. They restrict PHP execution only to files from a certain namespace
   ...
   :code:`user_` or :code:`tx_` namespace).
...
named like :code:`class.t3lib_cache_backend_abstractbackend.php`.
...
must start with :code:`class.tx_` prefix, followed by the extension key
...
example, if the extension key is :code:`test_ext`, the file name will be
...
single installation, although the :code:`user_` prefix is not recommended
here.

Non–class files contain PHP functions. These functions can be called
by TYPO3 only if they have :code:`user_` prefix (the prefix can be changed by
the administrator in the Install tool). All functions inside such files
must have the :code:`user_` prefix as well. Usually such files are placed
