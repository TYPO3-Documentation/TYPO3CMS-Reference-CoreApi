...
======================================
Guidelines for `TYPO3`:pn: integrators
======================================
...
A `TYPO3`:pn: integrator develops the template for a website, selects,
...
usually has "administrator" access to the `TYPO3`:pn: system, should have a
good knowledge of the general architecture of `TYPO3`:pn: (frontend,
backend, extensions, `TypoScript`:pn:, `TSconfig`:pn:, etc.) and should be able to
configure a `TYPO3`:pn: system properly and securely.
...
structure of files and directories used by `TYPO3`:pn:.

The installation of `TYPO3`:pn: on a web server or the configuration of the
...
The `TYPO3`:pn: integrator knows how to configure a `TYPO3`:pn: system, handed
over from a system administrator after the installation. An integrator
usually consults and trains editors (end-users of the system, for example a
client) and works closely together with system administrators.

The role of a `TYPO3`:pn: integrator often overlaps with a system
...
also apply for a `TYPO3`:pn: integrator. One of the most important rules is to change the
username and password of the "admin" account immediately after a `TYPO3`:pn: system
...
In addition, the following general rules apply for a `TYPO3`:pn: integrator:
...
#. Ensure, the `TYPO3`:pn: sites they are responsible for, always run a stable
   and secure `TYPO3 Core`:pn:  version and always and only contain secure extensions
   (integrators update them immediately if a vulnerability has been
   discovered).

#. Stay informed about `TYPO3 Core`:pn:  updates. Integrators should know the
   changes when new `TYPO3`:pn: major versions are released and should be aware
