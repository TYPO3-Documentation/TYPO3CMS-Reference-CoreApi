...
errors ranks "SQL injection" first! The `TYPO3 Security Team`:pn: comes across this security
...
you do not control without proper verification and validation (for example
...
`TYPO3`:pn: outputs the `JavaScript`:pn: code in :typoscript:`page.10.20.value` on the page.
...
code (for example `JavaScript`:pn:) can be placed in the HTML content of a page by
using `TypoScript`.
...
`TYPO3`:pn: allows to include external files which implement `TypoScript`
...
to `TYPO3`:pn:. A typical line to include an external `TypoScript`:pn: file looks like this:
...
following ```Apache`:pn: configuration could be used::
...
case that an attacker manages to manipulate these files (for example via a
compromised FTP account), it is possible to compromise the TYPO3 system or
to place malicious code (for example XSS) in the output of the pages generated by the CMS.
...
.. note::
    ...
    :file:`fileadmin/` is for content only, managed by editors, but `TypoScript`:pn: as well
    as HTML templates, `JavaScript`:pn:, CSS and other types of files are "system" or "project"
...
The following `TypoScript`:pn: adds the appropriate line to the HTTP header:
...
Integrity of external `JavaScript`:pn: files
============================================

The `TypoScript`:pn: property :code:`integrity` has been introduced with TYPO3 v7. This
configuration allows integrators to specify a SRI hash in order to allow a verification
of the integrity of externally hosted `JavaScript`:pn: files. SRI (Sub-Resource Integrity) is a
...
The `TypoScript`:pn: property can be used for the following :ref:`PAGE <t3tsref:page>` properties:
...
A typical example in `TypoScript`:pn: looks like:
...
Risk of externally hosted `JavaScript`:pn: libraries
=====================================================

In many cases, it makes perfect sense to include `JavaScript`:pn: libraries, which are
...
However, `JavaScript`:pn: libraries of any kind and nature, for example feedback,
...
If you include a `JavaScript`:pn: library that is hosted under
...
`JavaScript`:pn: running in the browser of your end-users is able to intercept any
...
externally hosted `JavaScript`:pn: files or to only include them on pages, where
