...
the web server runs as (for example `www-data`). On some systems (for example shared
...
their websites (for example various clients on a shared server) is to ensure
...
possible to revoke any write permissions for the web server user (for example
...
On `Unix`:pn: / `Linux`:pn: based systems, a secure configuration can be achieved by
