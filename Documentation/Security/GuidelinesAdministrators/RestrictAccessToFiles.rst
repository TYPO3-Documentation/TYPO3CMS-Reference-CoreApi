...
TYPO3 installations may use either a classic mode (non-Composer) or a
...
set to the :file:`public/` directory. All web-accessible files are explicitly
placed within this folder, while all sensitive or internal application files
...
-   Ensure the web server’s document root points only to the :file:`public/`
    directory.
-   Verify that all non-public files (e.g., :file:`composer.json`, :file:`.env`,
    :file:`vendor/`, :file:`config/`) reside outside of this directory.
-   Keep your :file:`public/.htaccess` (Apache) or server config (NGINX/IIS)
    updated to deny access to remaining critical files inside the public folder.
-   Store downloadable files intended only for authenticated users in
    `File storages <https://docs.typo3.org/permalink/t3coreapi:fal-administration-storages>`_
...
In classic TYPO3 installations (without Composer), all files reside inside the
...
accidentally placed sensitive files in the document root.
...
For classic mode, TYPO3 automatically writes default web server config files
...
regularly compare their config files with TYPO3’s reference templates:
...
TYPO3 cannot install default protection automatically. Instead, administrators
