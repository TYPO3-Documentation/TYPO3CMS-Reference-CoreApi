...
file mounts (for example `fileadmin/` in most cases). This also includes the
possibility to upload potential malicious code in HTML or SVG files
(using `JavaScript`:pn:, injecting cross-site scripting vulnerabilities).
...
accessed via https://example.org/fileadmin/...::
   ...
   # placed in fileadmin/.htaccess on `Apache`:pn: 2.x webserver
