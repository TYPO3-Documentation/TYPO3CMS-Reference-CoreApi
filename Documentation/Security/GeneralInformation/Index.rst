...
`TYPO3`:pn: versions and lifecycle
==================================

`TYPO3`:pn: is offered in Long Term Support (LTS) and Sprint Release versions.
...
`TYPO3`:pn: `9.0.0` was the first Sprint Release of the `9` branch and its support
ended when `TYPO3`:pn: `9.1.0` got released.
...
`TYPO3`:pn: version 8 (`v8`) and v9 are such LTS versions.

Starting with `TYPO3`:pn: v7 the minor-versions are skipped in the official
...
preceding LTS release. For example, when `TYPO3`:pn: v9 is the current LTS release,
`TYPO3`:pn: v8 is still actively supported, including security updates.

For users of v8 an update to v9 is recommended. All versions below `TYPO3`:pn: v8 are
...
In cases where users can't yet upgrade to a supported version, the `TYPO3`:pn: GmbH is offering
...
`TypoScript`:pn:, templates or `CSS` code may be necessary. With LTS and
Sprint Releases also the system requirements (for example PHP or `MySQL`:pn:
...
Difference between `Core`:pn: and extensions
============================================

The `TYPO3`:pn: base system is called the `Core`:pn:. The functionality of the
`Core`:pn: can be expanded, using extensions. A small, selected number of
extensions (the system extensions) are being distributed as part of
the `TYPO3 Core`:pn:. The `Core`:pn: and its system extensions are being developed
by a relatively small team (40-50 people), consisting of experienced
and skilled developers. All code being submitted to the `Core`:pn: is
reviewed for quality by other `Core Team`:pn: members.

Currently there are more than 5500 extensions available in the `TYPO3
Extension Repository`:pn: (TER), written by some 2000 individual
...
known security issues in `TYPO3`:pn: have been found in these extensions,
which are not part of the `Core`:pn: system.
...
Information about new `TYPO3`:pn: releases as well as security bulletins are
being announced on the "TYPO3 Announce" mailing list. Every system
administrator who hosts one or more `TYPO3`:pn: instances, and every `TYPO3`:pn:
integrator who is responsible for a `TYPO3`:pn: project should subscribe to
...
contain important information about `TYPO3`:pn: releases and security
bulletins.
...
When security updates for `TYPO3`:pn: or an extension become available, they
...
official `TYPO3 Security Team`:pn: website at
`https://typo3.org/help/security-advisories/
<https://typo3.org/help/security-advisories/>`_

Security bulletins for the `TYPO3 Core`:pn:  are separated from security
bulletins for `TYPO3`:pn: extensions. Every bulletin has a unique advisory
identifier such as TYPO3-CORE-SA-yyyy-nnn (for bulletins applying to
the `TYPO3 Core`:pn: ) and TYPO3-EXT-SA-yyyy-nnn (for bulletins applying to
`TYPO3`:pn: extensions), where yyyy stands for the appropriate year of
publication and nnn for a consecutively increasing number.

The bulletins contain information about the versions of `TYPO3`:pn: or
versions of the extension that are affected and the type of security
issue (for example information disclosure, cross-site scripting, etc.). The
...
For `TYPO3`:pn: extensions, there are two types of security bulletins:
...
For some critical security issues the `TYPO3 Security Team`:pn: may decide
...
Security issues in the `TYPO3 Core`:pn:  which are only exploitable by users
...
standard `Core`:pn: review process. This implies that the development of the
...
Important security related information regarding `TYPO3`:pn: products or the
...
software like such as `Apache`:pn: , `nginx`:pn:, `MySQL`:pn:, PHP, etc. that are related
to `TYPO3`:pn: products, possible security related misconfigurations in third
party software, possible misconfigurations in `TYPO3`:pn: products, security
related information about the server infrastructure of typo3.org and
other important recommendations how to securely use `TYPO3`:pn: products.
...
Since 2010 the `TYPO3 Security Team`:pn: also publishes a CVSS rating with
