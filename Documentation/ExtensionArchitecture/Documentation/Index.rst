...
If you plan to upload your extension to the `TYPO3 Extension Repository`:pn: (TER), you should first consider adding
...
called `reStructuredText`:pn: (`reST`:pn:).
...
- **Cross-references:** It is easy to cross-reference other chapters and sections of other manuals (either `TYPO3`:pn:
...
- The `Extension Builder <https://extensions.typo3.org/extension/extension_builder>`_
  provides a skeleton documentation based on the above-mentioned `Git`:pn: repository.
...
using `reST`:pn:, because that is what is commonly used in TYPO3 documentation projects and that is what is
supported best.
...
of this file is `reStructuredText`:pn:, as for chapters of a Sphinx project.
