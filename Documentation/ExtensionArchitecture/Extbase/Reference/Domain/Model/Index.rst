...
..  seealso::
    ...
    command `vendor/bin/typo3 make:model` to create an Extbase model for an
    existing `database table <https://docs.typo3.org/permalink/t3coreapi:database-records>`_,
    which can in turn be generated by command `vendor/bin/typo3 make:table`.
...
One model is always connected to one database table (with the according TCA
definition), however one table can be represented by more then one domain models.

For example you can have one model for each type that the table supports or
specialized models only used for certain views.
