...
value. This page lists all converters shipped with TYPO3, their accepted
...
automatically whenever an action parameter is typed as :php:`int`,
:php:`float`, :php:`bool`, :php:`string`, or :php:`array`.
...
:php:`PersistentObjectConverter::CONFIGURATION_MODIFICATION_ALLOWED`
    ...
    has a UID. Required when an array payload includes an ``__identity``
...
:php:`PersistentObjectConverter::CONFIGURATION_CREATION_ALLOWED`
    ...
    that carries no ``__identity`` key.

Both constants default to :php:`false`. When a ``__trustedProperties``
