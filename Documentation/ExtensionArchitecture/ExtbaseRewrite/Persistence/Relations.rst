...
Domain objects rarely stand alone: a :php:`Conference` has a :php:`Location`,
holds many :php:`Comment` objects, and is staffed by several :php:`Speaker`
objects who in turn speak at many conferences. Extbase maps these object
relations onto the relational database and loads the related objects for you
when you access the property — no :sql:`JOIN` in your code.

This page explains how each relation **cardinality** is stored and loaded, and
the performance trade-off that lazy loading introduces. How to **declare**
relations on the model — the property types, :php:`#[Lazy]`, :php:`#[Cascade]`,
the :php:`ObjectStorage` getter/setter pattern — is covered with the model:
...
A relation's *cardinality* is how many objects sit on each side. Throughout this
...
The owning object relates to exactly one other — a :php:`Conference` has one
:php:`Location`. On the model this is a single typed, usually nullable, property.

In the database the relation is stored as a single column on the owning table
holding the UID of the related record (for example a :sql:`location` column on
...
..  note::
    ...
    A single related object on the model can express several intentions that look
    ...
    Extbase itself does not enforce uniqueness on the related side.
...
part of its conference. On the model this collection is an
:php:`\TYPO3\CMS\Extbase\Persistence\ObjectStorage`, Extbase's container for
related objects.

In the database the *child* records carry a column pointing back at the parent
...
The owning object holds a collection of others that exist independently, and the
relationship runs both ways — a :php:`Conference` has many :php:`Speaker`
objects, and each speaker appears at many conferences. Deleting one conference
must not delete the speaker. On the model the collection is again an
:php:`ObjectStorage`.

In the database an m:n relation is normally stored in a dedicated **intermediate
...
directly. TCA expresses this with
:ref:`type="select" <t3tca:columns-select>` or
:ref:`type="group" <t3tca:columns-group-introduction>` plus an :sql:`MM`
setting naming the intermediate table. The MM table is the recommended form: it
...
a single column on the owning table (the same :sql:`type=select` / :sql:`group`
field *without* an :sql:`MM` setting). This avoids the extra table but does not
scale, is hard to query, and only records the relation on one side. Prefer the
MM table for any genuine m:n relation; the CSV form is acceptable for short,
...
belongs to other conferences. Removing an object from the :php:`ObjectStorage`
deletes the row in the MM table, not the speaker record.
...
holds ready-to-use objects. You access :php:`$conference->getLocation()` or
iterate :php:`$conference->getComments()` and the objects are simply there.

The order of children inside a relation is **not** controlled by a repository
query — there is no query to override, because Extbase loads the children as
...
..  seealso::
    ...
    `Ordering in Extbase relations <https://docs.typo3.org/permalink/extbase-domain-repository-ordering-relations>`_ — why repository orderings do not reach relation children.
...
Eager loading is convenient but not free. Loading a list of 50 conferences,
each with an eagerly loaded :php:`ObjectStorage` of comments, means Extbase
loads every comment of every conference up front — even if the list view never
displays them.

Marking the relation :php:`#[Lazy]` defers it: Extbase installs a lightweight
...
opposite — the **N+1 query problem**. Consider a list template that *does* touch
...
With a lazy relation this runs **one** query to fetch the 50 conferences, then
**one more query per conference** to load its comments when the template
accesses them — 51 queries for one page. The relation being lazy did not save
work here; it spread the same work across many small queries, which is usually
slower than one larger one.

The rule of thumb:

*   **Eager (the default)** when you almost always use the related objects —
    a detail view that always renders the location and all comments.
*   **Lazy** (:php:`#[Lazy]`) when you usually *do not* touch the relation —
    a list view that shows only titles, where loading every child would be
    wasted work.
*   When a list view *does* need related data for every row, neither extreme is
    ideal. The honest answer is often to step outside the ORM and fetch exactly
    what the view needs in a single query.

..  seealso::

    `When to drop out of the ORM <https://docs.typo3.org/permalink/extbase-domain-repository-dbal>`_ — using raw DBAL from a repository when relation loading becomes a performance problem.
...
..  seealso::
    ...
    `Modelling relations in Extbase <https://docs.typo3.org/permalink/extbase-domain-model-relations>`_ — declaring relations on the model.

With relations and queries understood, you have the full persistence picture an
Extbase extension needs. For everything the ORM deliberately does not do —
