...
on the version of`TYPO3 CMS`:pn: which you are targeting. The `TCA`:pn: can only be
...
case you are modifying `Core`:pn: `TCA`:pn:, you usually don't have to worry about that**. Since
custom extensions are always loaded *after* the `Core's`:pn: `TCA`:pn:, changes from custom extensions
...
Since`TYPO3 CMS`:pn: 6.2 (6.2.1 to be precise) changes to :php:`$GLOBALS['TCA']`
...
Thus if you want to customize the `TCA`:pn: of :code:`tx_foo_domain_model_bar`,
...
.. note::
   ...
   Also names don't matter in that folder, at least not to `TYPO3`:pn:. They only might influence
   loading order. Proper naming is only relevant for the real definition of tables one
   folder up in :file:`Configuration/TCA`

.. important::

   Be aware that you cannot extend the `TCA`:pn: of extensions if it was configured within
...
.. important::
   ...
   Only `TCA`:pn:-related changes should go into :file:`Configuration/TCA/Overrides`
...
Until`TYPO3 CMS`:pn: 6.1 (still supported for 6.2) changes to :php:`$GLOBALS['TCA']` are packaged
into an extension's :file:`ext_tables.php` file. This is strongly discouraged in more recent
versions of`TYPO3 CMS`:pn:.

Nowadays the only usecase for `TCA`:pn: changes in :file:`ext_tables.php` is to override `TCA`:pn: definitions
done in the :file:`ext_tables.php` of a legacy extension. `TCA`:pn: overrides cannot be used in this case
...
Changing the `TCA`:pn: "on the fly"
===================================
