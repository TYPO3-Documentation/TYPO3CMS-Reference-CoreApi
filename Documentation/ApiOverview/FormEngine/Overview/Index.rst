...
via `FormEngine`:pn:, and hand over any given data by a `FormEngine`:pn: :code:`POST` result over to the `DataHandler`:pn:
to persist stuff in the database.

The rendering part of the :php:`EditDocumentController` job splits into these parts:

* Initialize main `FormEngine`:pn: data array using :code:`POST` or :code:`GET` data to specify which specific record(s)
  should be edited.

* Select which group of DataProviders should be used.

* Trigger `FormEngine`:pn: DataCompiler to enrich the initialized data array with further data by calling all data
  providers specified by selected data provider group.

* Hand over DataCompiler result to an entry "render container" of `FormEngine`:pn: and receive a result array.

* Take result array containing HTML, CSS and `JavaScript`:pn: details and put them into :php:`FormResultCompiler` which
...
.. figure:: ../../../Images/FormEngineMainWorkflow.svg
   :alt: Main `FormEngine`:pn: workflow

The controller does two distinct things here: First, it initializes a data array and lets it get enriched by
data providers of `FormEngine`:pn: which add all information needed for the rendering part. Then feed this data array
to the rendering part of `FormEngine`:pn: to end up with a result array containing all HTML, CSS and `JavaScript`:pn:.
...
This basically means the main `FormEngine`:pn: concept is a two-fold process: First create an array to gather all
...
* Data providing and rendering is split: Controllers could re-use the rendering part of `FormEngine`:pn: while
  all or parts of the data providers are omitted, or their data comes from "elsewhere". Furthermore, controllers can
  re-use the data providing part of `FormEngine`:pn: and output the result in an entirely different way than HTML. The
  latter is for instance used when `FormEngine`:pn: is triggered for a `TCA`:pn: tree by an ajax call and thus outputs a JSON array.
