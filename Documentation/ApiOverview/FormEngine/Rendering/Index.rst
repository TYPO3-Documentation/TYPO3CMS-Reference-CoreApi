...
Complete Example for adding a fieldControl to a pages field
------------------------------------------------------------

To illustrate the principals discussed in this chapter see the following example which registers a fieldControl (button) next to a field
in the pages table to trigger a data import via ajax.
...
Add the php class for rendering the control in :file:`FormEngine/FieldControl/ImportDataControl.php`:
...
Add the JavaScript for defining the behavior of the control in :file:`Resources/Public/JavaScript/ImportData.js`:
...
Add an ajax route for the request in :file:`Configuration/Backend/AjaxRoutes.php`:
...
Add the ajax controller class in :file:`Classes/Controller/Ajax/ImportDataController.php`:
