...
Composer installations only.  The folder :file:`config/sites` contains
...
Composer installations only.  Each web site which is run on TYPO3 **should**
have a sitepackage, anextension with a special purpose containing all
...
installations by default all files are located in this folder.
...
The files are update each time an extension is installed in the
...
Legacy installations only. Directory for extension localisations.
...
Legacy installations only. Contains contains
subfolders for each :ref:`site configuration <sitehandling>`.
...
files (e.g. cached Fluid templates) and should not be publicly available.
...
Composer installations only. Directory for temporary files that contains private files (e.g.
...
Composer installations only. This directory contains internal files needed
for the cache.
...
Composer installations only.  The directory :file:`var/labels/` is for extension
localisations. It contains all downloaded translation files.
...
Composer installations only.  This directory contains log files like the
...
Composer installations only.  In this directory, which lies outside of
