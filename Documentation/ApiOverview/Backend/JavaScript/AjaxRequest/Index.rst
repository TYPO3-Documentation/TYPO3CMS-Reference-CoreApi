...
..  note::
    ...
    :js:`{credentials: 'same-origin', signal: AbortController.signal}`.

The body of the request is automatically converted to a `FormData`_ object, if
...
In the examples above :js:`promise` is, as the name already spoils, a `Promise`_
...
(:js:`@typo3/core/ajax/ajax-response.js`). The object is a simple wrapper for
the original `Response`_ object. :js:`AjaxResponse` exposes the following
...
..  hint::
    ...
    as "successful", but sets the response's :js:`ok` field to :js:`false`. The
