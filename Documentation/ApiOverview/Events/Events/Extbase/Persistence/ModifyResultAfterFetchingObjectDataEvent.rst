..  include:: /Includes.rst.txt
..  index:: Events; ModifyResultAfterFetchingObjectDataEvent
..  _ModifyResultAfterFetchingObjectDataEvent:

========================================
ModifyResultAfterFetchingObjectDataEvent
========================================

The PSR-14 event
:php:`\TYPO3\CMS\Extbase\Event\Persistence\ModifyResultAfterFetchingObjectDataEvent`
is fired after the storage backend has pulled results from a given query.

..  _modify-result-after-fetching-object-data-event-example:

Example
=======

..  include:: /_includes/EventsContributeNote.rst.txt

..  _modify-result-after-fetching-object-data-event-api:

API
===

..  include:: /CodeSnippets/Events/Extbase/ModifyResultAfterFetchingObjectDataEvent.rst.txt
