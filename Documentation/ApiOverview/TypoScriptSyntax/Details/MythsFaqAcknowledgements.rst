...
Myth: "`TypoScript`:pn: is a scripting language"
================================================

This is misleading to say since you will think that `TypoScript`:pn: is like
PHP or `JavaScript`:pn: while it is not. From the previous pages you have
learned that `TypoScript`:pn: strictly speaking is just a syntax. However
when the `TypoScript`:pn: syntax is applied to create `TypoScript`:pn: templates
then it begins to look like programming.

In any case `TypoScript`:pn: is **not** comparable to a scripting language like
PHP or `JavaScript`:pn:. In fact, if `TYPO3 CMS`:pn: offers any scripting language it
is PHP itself! `TypoScript`:pn: is only an API which is often used to
...
`TypoScript`:pn: is still not a procedural programming language!
...
*information*) you can say that `TypoScript`:pn: is a proprietary syntax
since a PHP array or XML file could be used to contain the same
information as `TypoScript`:pn: does. But this is *not* a drawback. For
...
anyways - `TypoScript`:pn: is much better at that job (see below).

To claim that `TypoScript`:pn: is a proprietary standard as an argument
against TYPO3 is really unfair since the claim makes it sound like
`TypoScript`:pn: is a whole new programming language or likewise. Yes, the
`TypoScript`:pn: *syntax* is proprietary but extremely useful and when you
get the hang of it, it is very easy to use. In all other cases `TYPO3`:pn:
uses official standards like PHP, SQL, XML, XHTML etc. for all
*external* data storage and manipulation.

The most complex use of `TypoScript`:pn: is probably with the `TypoScript`:pn:
Template Records. It is understandable that `TypoScript`:pn: has earned a
reputation of being complex when you consider how much of the Frontend
Engine you can configure through `TypoScript`:pn: template records. But
basically `TypoScript`:pn: is just an API to the PHP functions underneath.
...
wouldn't have the streamlined abstraction provided by `TypoScript`:pn:
...
`TypoScript`:pn: was not invented!
...
- Disciplined coding: Organize your `TypoScript`:pn: in a way that you can
  visually comprehend.
...
A few times `TypoScript`:pn: has been compared with XML since both
"languages" are frameworks for storing information. Apart from XML
being a W3C standard (and `TypoScript`:pn: still not... :-) ) the main
difference is that XML is great for large amounts of information with
a high degree of "precision" while `TypoScript`:pn: is great for small
amounts of "ad hoc" information - like configuration values normally
are.

Actually a data structure defined in `TypoScript`:pn: could also have been
modeled in XML. Currently you *cannot* use XML as an alternative to
TypoScript, but this may happen at some
point. Let's present this fictitious example of how a `TypoScript`:pn:
structure could also have been implemented in "TSML" (our fictitious
name for the non-existing `TypoScript`:pn: Mark-Up Language):
...
That was 17 lines of `TypoScript`:pn: code and converting this information
...
`TypoScript`:pn: normally does. But hopefully you can at least use this
example in your understanding of what `TypoScript`:pn: is compared to XML.

The reasonable application for using XML as an alternative solution to
`TypoScript`:pn: is if an XML editor existed which in some way made the
