...
Page-based routing is always enabled in TYPO3 and requires a site configuration
(see :ref:`Site handling <sitehandling>`) for your website. Each page's route is
determined by its `slug` field, which can be viewed in the page properties.
...
(:ref:`Slugs / URL parts <t3tca:columns-slug>`) for available options.
