...
`TYPO3`:pn: contains various SEO related functionality out of the box. The following provides an introduction in those features.
...
Canonical Tags
    ...
    If the `Core`:pn: extension "seo" is installed, it will automatically add the canonical link to the page.
    ...
    Should an empty href occur when generating the link to overwrite the canonical (this happens for example if the
...
.. warning::
    ...
    If both the `Core`:pn: and an extension are generating a canonical link, it will
