...
Public resources that have to be available in the web like CSS and JavaScript
...
folder :folder:`Resources/Private` or :folder:`Resources/Configuration` of an
extension or the folders :folder:`config/sites` or :folder:`config/system`.

Images and Downloads may reside in folder :folder:`public/fileadmin` or a custom
file storage.
...
example by using :file:`.htaccess` or Nginx configurations. The earlier can
...
They resolve the path internally. `my_extension` is extension key as defined in
`extra.typo3/cms.extension-key <https://docs.typo3.org/permalink/t3coreapi:ext-composer-json-property-extension-key>`_
in the extensions :file:`composer.json`, the rest is the relative path from that
extensions root folder.
...
can be used to resolve path to other locations within the project such as the
