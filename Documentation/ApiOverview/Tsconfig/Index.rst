...
User `TSconfig`:pn: and page `TSconfig`:pn: are very flexible concepts for
adding fine-grained configuration to the backend of the`TYPO3 CMS`:pn:. It is a text-
based configuration system where you assign values to keyword strings,
using the `TypoScript`:pn: syntax. The :ref:`TSconfig Reference <t3tsconfig:start>`
...
User `TSconfig`:pn: can be set for each backend user and group.
...
Page `TSconfig`:pn: can be set for each page in the page tree. Pages
...
When there is the necessity for fetching and loading page `TSconfig`:pn:,
...
The string can then be put in proper `TSconfig`:pn: array syntax::
