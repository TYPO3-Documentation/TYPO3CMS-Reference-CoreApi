...
.. warning::
    The `PSR-7` Request and the `Extbase`:pn: request are different things. You cannot
    access the site configuration via the `Extbase`:pn: request. When in `Extbase`:pn: context
