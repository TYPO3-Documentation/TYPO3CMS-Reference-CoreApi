...
The `TYPO3 Core Engine`:pn: is the class that handles *all* ***data***
...
Using `TCE`:pn: for manipulation of the database content in the $TCA-configured
...
`TCE`:pn: requires a backend login to work. This is due to the fact that
permissions are observed (of course) and thus `TCE`:pn: needs a backend user
to evaluate against. This means you cannot use `DataHandler`:pn: from the
frontend scope. Thus writing to tables (such as a guestbook) will have
to be done from the frontend *without* `DataHandler`:pn:.
...
`TCE`:pn: also has a part for handling files. The file operations are
...
and directories. The file operations are managed by two `Core`:pn: classes,
