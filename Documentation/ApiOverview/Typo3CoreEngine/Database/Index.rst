...
* A page UID prefixed with `pageID_` (`pageId_<page-uid>`), e.g. `pageId_10` when updating a page with UID 10 (additionally all related pages, see :ref:`clearcache-pagegrandparent` and :ref:`clearcache-pagesiblingchildren`) and `pageId_10` when updating a record if a record of any table placed on the page with UID 10 (`<table>.pid = 10`) is updated.
