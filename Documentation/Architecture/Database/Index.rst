...
identifier
  ...
  file, relative to the storage root.
...
basic information stored in "sys\_file", the content of table
...
file
  Id of the file the metadata is related to.
...
title
  When a file is referenced, one would normally then use its title (for
  ...
  Fields "description", "alternative" and "downloadname" obey the same principle.
...
as it is only written to using direct SQL queries in the source code.
...
type
  The type of Collection. A Collection can be based on hand-picked files,
...
configuration
  ...
  :ref:`FlexForm field <t3tca:columns-flex>` and the actual options
