...
`TCA <https://docs.typo3.org/typo3cms/TCAReference/Index.html>`__ definitions that
...
`TCA reference <https://docs.typo3.org/typo3cms/TCAReference/Reference/Ctrl/Index.html>`__
...
* Automatic query upgrades: If a table was designed without soft-delete in the first
...
* Handing over restriction details to the framework: Having the restriction expressions
...
* Flexible: The class construct is created in a way that allows developers to extend
...
* `DeletedRestriction`: (default) Evaluates `['ctrl']['delete']`, adds for instance `AND deleted = 0` if
  `TCA['aTable']['ctrl']['delete'] = 'deleted'` is specified.

* `HiddenRestriction`: (default) Evaluates `['ctrl']['enablecolumns']['disabled']`, adds `AND hidden = 0`
  if `hidden` is specified as field name.

* `StarTimeRestriction`: (default) Evaluates `['ctrl']['enablecolumns']['starttime']`, typically adds
...
* `BackendWorkspaceRestriction`: Determines the current workspace a backend user is working in and adds
...
The default restrictions are often enough. Nothing needs to be done in those cases.
...
`QueryRestrictionInterface` can be given to `->add()`. This allows extensions to deliver own restrictions.

An alternative to the recommended way of first removing all restrictions and then adding needed
ones again (using `->removeAll()`, then `->add()`) is to kick specific restrictions with a call to
`->removeByType()`:
...
Note that `->setRestrictions()` resets any previously specified restrictions. Any class instance implementing
`QueryRestrictionContainerInterface` can be given to `->setRestrictions()`. This allows extensions to
...
.. note::
    ...
    `debug($queryBuilder->getSQL())` right before the final call to `$queryBuilder->execute()`. Just
    take care these calls do :ref:`not end up in production <database-query-builder-get-sql>` code.